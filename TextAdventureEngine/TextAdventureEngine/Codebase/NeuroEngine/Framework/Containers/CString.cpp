...
//=====================================================================================
CString::CString( const char * a_String, uint32_t a_Repeat )
	: CString()
{
	for ( uint32_t i = 0; i < a_Repeat; ++i )
	{
		*this += a_String;
	}
}
...
Array< CString::RegexMatch > CString::Regex( const char * a_Pattern ) const
{
	...
	try
	{
		reg = std::regex( a_Pattern, std::regex::optimize );
	}
	
	catch ( const std::regex_error & err )
	{
		const char * e = err.what();
		PRINT( "CString Regex Error: %s", e );
		return Array< CString::RegexMatch >();
	}
...
bool CString::Parse( const char * a_Value, float & a_Out )
{
	...
	if ( ( f = static_cast< float >( atof( a_Value ) ) ) == 0.0F )
	{
