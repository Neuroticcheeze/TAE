...
public:
	...
	void InitPost();
	void StartGame( Functor< void( float ) > a_GameLoopEvent );
	void Finalise();
	float GetTime() const;
	...
	inline void * GetGLContext() const { return m_GlCtx; }
	inline void * GetHDC() const { return m_hDC; }

private:

	CString m_Title;
	Vector2 m_DisplaySize;
	Functor< void(float) > m_ClientCallback = nullptr;
