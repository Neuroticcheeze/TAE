...
{
	m_HVPass = GraphicsManager::Instance().LoadShader( CRC32( "GaussianBlurHVPass" ), "gaussian_blur.shader", CRC32( "MAIN" ), CRC32( "HVPASS" ) );
	
	m_HVPassProp_SrcTex = GraphicsManager::Instance().GetShaderGlobalLocation( m_HVPass, "uPrevPassResult" );

	m_FullQuad = GLMeshIndexed::CreateQuad( "GaussianBlurFullQuad", -Vector2::ONE );
}
