...
#include <Framework/Graphics/GL/GLMeshIndexed.hpp>
...
void GraphicsManager::InitPost()
{
	...
	m_DefaultFont = LoadFont( CRC32( "_Internal_DefaultFont" ), "builtin/default.ttf", 32 );

	m_TextShader = LoadShader( CRC32( "_Internal_TextShaderPerfectEdges" ), "builtin/text.shader", CRC32( "Text_VMain" ), CRC32( "Text_FPerfectEdges" ) );
	m_TextShaderGradiented = LoadShader( CRC32( "_Internal_TextShaderPerfectEdgesGradiented" ), "builtin/text.shader", CRC32( "Text_VMain" ), CRC32( "Text_FPerfectEdgesGradiented" ) );
	m_TextShaderGlyphs = LoadShader( CRC32( "_Internal_TextShaderGlyphPerfectEdges" ), "builtin/text.shader", CRC32( "Text_VGlyph" ), CRC32( "Text_FPerfectEdges" ) );
	m_TextShaderGlyphsGradiented = LoadShader( CRC32( "_Internal_TextShaderGlyphGradientedPerfectEdges" ), "builtin/text.shader", CRC32( "Text_VGlyph" ), CRC32( "Text_FPerfectEdgesGradiented" ) );
	m_SimpleColourShader = LoadShader( CRC32( "_Internal_SimpleColourShader" ), "builtin/colour.shader", CRC32( "Col_VMain" ), CRC32( "Col_FMain" ) );
	m_TextureColourExplicitUVShader = LoadShader( CRC32( "_Internal_TextureColourExplicitUVShader" ), "builtin/texture.shader", CRC32( "Col_VExplicitUV" ), CRC32( "Col_FTextureColour" ) );
	m_VQuadShader = LoadShader( CRC32( "_Internal_VQuad" ), "builtin/vquad.shader", CRC32( "VQuad_VMain" ), CRC32( "VQuad_FMain" ) );
	m_BezierVQuadShader = LoadShader( CRC32( "_Internal_BezierVQuad" ), "builtin/vquad_bezier.shader", CRC32( "VMain" ), CRC32( "FMain" ) );
	m_BezierVQuadDashedShader = LoadShader( CRC32( "_Internal_BezierVQuadDashed" ), "builtin/vquad_bezier.shader", CRC32( "VMainDashed" ), CRC32( "FMainDashed" ) );
	m_TriShader = LoadShader( CRC32( "_Internal_TriShader" ), "builtin/colour.shader", CRC32( "Col_VExplicit" ), CRC32( "Col_FExplicit" ) );
	m_SimpleTextureColourShader = LoadShader( CRC32( "_Internal_SimpleTextureColourShader" ), "builtin/texturecolour.shader", CRC32( "VSimpleTextureColour" ), CRC32( "FSimpleTextureColour" ) );
	m_SimpleColour2Shader = LoadShader( CRC32( "_Internal_SimpleColour2Shader" ), "builtin/colour.shader", CRC32( "Col_VColVertSrc" ), CRC32( "Col_FColVertSrc" ) );
	m_SimpleTextureShader = LoadShader( CRC32( "_Internal_SimpleTextureShader" ), "builtin/texture.shader", CRC32( "VSimpleTexture" ), CRC32( "FSimpleTexture" ) );

	m_BadStringTexture = ResolveTextTexture( "[ERROR]", m_DefaultFont, false );

	m_01Quad = GLMeshIndexed::CreateQuad( "_Internal_01Quad", Vector2::ZERO, Vector2::ONE );
	m_01LineQuad = new GLMeshIndexed( "_Internal_01LineQuad", VFD_POS2_TEX2, {
						0.0F, 0.0F, 0.0F, 0.0F, 
						0.0F, 1.0F, 0.0F, 0.0F, 
						1.0F, 1.0F, 0.0F, 0.0F, 
						1.0F, 0.0F, 0.0F, 0.0F },
						{ 0, 1, 1, 2, 2, 3, 3, 0 }, 
						GLMeshIndexed::DrawMode::DM_LINES );
	m_BezierQuadStrip128 = GLMeshIndexed::CreateGrid( "_Internal_BezierQuad128", 1, 128 );
	m_Triangle = new GLMeshIndexed( "_Internal_Triangle", VFD_POS2, {
						0.0F, 0.0F, 
						0.0F, 0.0F, 
						0.0F, 0.0F },
						{ 0, 1, 2 }, 
						GLMeshIndexed::DrawMode::DM_TRIANGLES );
...
void GraphicsManager::Finalise()
{
	...
	UnloadShader( m_SimpleTextureColourShader );
	UnloadShader( m_SimpleColour2Shader );
	UnloadShader( m_SimpleTextureShader );
...
GraphicsManager::Shader GraphicsManager::LoadShader( uint32_t a_HashName, const char * a_Path, uint32_t a_VertexShader, uint32_t a_FragmentShader )
{
	...
	if ( !m_Shaders.Contains( a_HashName ) )
	{
		...
		while ( it < end && !( foundFSrc && foundVSrc ) )
		{
			...
			uint32_t shaderName = CRC32( shaderNameS.Get() );
					 ...
					 ( a_VertexShader == 0 && shaderNameS.Length() == 0 ) )
				{
					...
					while ( loaderIt != loaderEnd )
					{
						runtimeTextures.Append( Tuple2< Texture, uint32_t >( 
							GraphicsManager::Instance().LoadTexture( CRC32( loaderIt->VarName.Get() ), loaderIt->Path.Get(), 
		...
		ConsoleManager::Instance().Praise( CString::Format( "Loaded Shader: \"%s\"", a_Path ) );
	}
...
GraphicsManager::Texture GraphicsManager::LoadTexture( uint32_t a_HashName, const char * a_Path, TextureResidency a_TextureResidency, TextureFilterMode a_Filter, TextureWrapMode a_Wrap, bool a_EnableMipmapping, TextureType a_TextureType )
{
	...
	{
		FileManager::Instance().SetWorkingDirectory( FileManager::UserLocation::EXECUTABLE, "res/textures/" );
		CString path = FileManager::Instance().GetWorkingDirectory( a_Path );
		SDL_Surface * surface = IMG_Load( path.Get() );

		if ( ASSERT( surface, "Failed to load texture [%s]: %s", path.Get(), IMG_GetError() ) )
		{
			...
			ConsoleManager::Instance().Praise( CString::Format( "Loaded Texture: \"%s\"", path.Get() ) );
...
//=====================================================================================
GraphicsManager::TextureAtlas GraphicsManager::LoadTextureAsAtlas( uint32_t a_HashName, const char * a_Path, const Vector2 & a_SpritesPerAxis, TextureFilterMode a_Filter )
{
	if ( !m_TextureAtlases.Contains( a_HashName ) )
	{
		m_TextureAtlases.Put( a_HashName, PTextureAtlas( SpriteSheet( LoadTexture( a_HashName, a_Path, TEXRES_VIDEO_MEMORY, a_Filter, TextureWrapMode::CLAMP ), a_SpritesPerAxis ) ) );
	}
...
GraphicsManager::Font GraphicsManager::LoadFont( uint32_t a_HashName, const char * a_Path, uint32_t a_FontSize )
{
	...
	{
		FileManager::Instance().SetWorkingDirectory( FileManager::UserLocation::EXECUTABLE, "res/fonts/" );

		CString path = FileManager::Instance().GetWorkingDirectory( a_Path );
		TTF_Font * font = TTF_OpenFont( path.Get(), a_FontSize );

		if ( ASSERT( font, "Failed to load font: %s [%s]", path.Get(), TTF_GetError() ) )
		{
			m_Fonts.Put( a_HashName, PFont( PFont::PtrType( path.Get(), font, a_FontSize ) ) );

			ConsoleManager::Instance().Praise( CString::Format( "Loaded Font: \"%s\"", path.Get() ) );
...
GraphicsManager::BitmapFont GraphicsManager::LoadBitmapFont( uint32_t a_HashName, const char * a_CharacterSet, const Font & a_Font, uint32_t a_FontSize )
{
	...
	if ( !m_BitmapFonts.Contains( a_HashName ) )
	{
		...
		if ( ASSERT( font, "Invalid font for BitmapFont [%u]", a_HashName ) )
		{
			...
			Texture bitmap = GfxGenTextStatic( charsetProper.Get(), font, static_cast< float >( a_FontSize ) );
			int32_t bitmapSizeX;
			int32_t bitmapSizeY;
			TTF_SizeText( reinterpret_cast< TTF_Font* >( font->Value1 ), charsetProper.Get(), &bitmapSizeX, &bitmapSizeY );
			Vector2 bitmapSize( static_cast< float >( bitmapSizeX ), static_cast< float >( bitmapSizeY ) );
			...
			{				
				...
				glyph.Value1 = Vector2( static_cast< float >( prevOff ), 0.0F ) / bitmapSize;
				glyph.Value2 = Vector2( static_cast< float >( width ), bitmapSize.y ) / bitmapSize;
			}
...
{
	o_Result[ 0 ] = a_Right[ 0 ] * a_Left[ 0 ] + a_Right[ 1 ] * a_Left[ 3 ] + a_Right[ 2 ] * a_Left[ 6 ];
	o_Result[ 3 ] = a_Right[ 3 ] * a_Left[ 0 ] + a_Right[ 4 ] * a_Left[ 3 ] + a_Right[ 5 ] * a_Left[ 6 ];
	o_Result[ 6 ] = a_Right[ 6 ] * a_Left[ 0 ] + a_Right[ 7 ] * a_Left[ 3 ] + a_Right[ 8 ] * a_Left[ 6 ];
	o_Result[ 1 ] = a_Right[ 0 ] * a_Left[ 1 ] + a_Right[ 1 ] * a_Left[ 4 ] + a_Right[ 2 ] * a_Left[ 7 ];
	o_Result[ 4 ] = a_Right[ 3 ] * a_Left[ 1 ] + a_Right[ 4 ] * a_Left[ 4 ] + a_Right[ 5 ] * a_Left[ 7 ];
	o_Result[ 7 ] = a_Right[ 6 ] * a_Left[ 1 ] + a_Right[ 7 ] * a_Left[ 4 ] + a_Right[ 8 ] * a_Left[ 7 ];
	o_Result[ 2 ] = a_Right[ 0 ] * a_Left[ 2 ] + a_Right[ 1 ] * a_Left[ 5 ] + a_Right[ 2 ] * a_Left[ 8 ];
	o_Result[ 5 ] = a_Right[ 3 ] * a_Left[ 2 ] + a_Right[ 4 ] * a_Left[ 5 ] + a_Right[ 5 ] * a_Left[ 8 ];
	o_Result[ 8 ] = a_Right[ 6 ] * a_Left[ 2 ] + a_Right[ 7 ] * a_Left[ 5 ] + a_Right[ 8 ] * a_Left[ 8 ];
}
...
Vector2 GraphicsManager::GfxDrawText( const char * a_String, const BitmapFont & a_BitmapFont, uint32_t a_StringLength, bool a_StringLengthAffectsTextAlignment, TextAlignment a_Horizontal, TextAlignment a_Vertical, TextFlags a_Flags )
{
	...
	while ( it != end )
	{
		...
		*( right + 6 ) = accum;
		...
		// This shader-passthrough & drawcall = ~0.3ms
...
void GraphicsManager::GfxDrawBezier( const Bezier & a_Bezier, float a_Thickness )
{
	...
	const Array< Vector2 > & cps = bez.GetControlPoints();
...
//=====================================================================================
void GraphicsManager::GfxDrawVertexBuffer( const VertexBuffer & a_VertexBuffer, int32_t a_SamplerUnit )
{
	if ( a_VertexBuffer.m_GPUBuffer )
	{
		Shader shader;

		if ( a_VertexBuffer.m_Format == VFD_POS2 || a_VertexBuffer.m_Format == VFD_POS2_COL4 )
		{
			shader = m_SimpleColour2Shader;
			SetShaderGlobalColour( shader, GetShaderGlobalLocation( shader, "uColour" ), m_Colour[ ( uint32_t )COL_PRIMARY ] );
		} else
		if ( a_VertexBuffer.m_Format == VFD_POS2_TEX2 )
		{
			shader = m_SimpleTextureShader;
			SetShaderGlobalColour( shader, GetShaderGlobalLocation( shader, "uColour" ), m_Colour[ ( uint32_t )COL_PRIMARY ] );
			SetShaderGlobalSampler( shader, GetShaderGlobalLocation( shader, "uTexture" ), a_SamplerUnit );
		} else
		if ( a_VertexBuffer.m_Format == VFD_POS2_TEX2_COL4 )
		{
			shader = m_SimpleTextureShader;
			SetShaderGlobalColour( shader, GetShaderGlobalLocation( shader, "uColour" ), m_Colour[ ( uint32_t )COL_PRIMARY ] );
			SetShaderGlobalSampler( shader, GetShaderGlobalLocation( shader, "uTexture" ), a_SamplerUnit );
		} else 
			ASSERT(false, __FUNCTION__ " -> Invalid format!");

		SetShaderGlobalFloat3x3( shader, GetShaderGlobalLocation( shader, "uTransform" ), TfGetTop() );
		SetShader( shader );

		GLDraw::Instance().DrawMesh( a_VertexBuffer.m_GPUBuffer );
	}
}
...
GraphicsManager::Texture GraphicsManager::ResolveTextTexture( const CString & a_Text, const Font & a_Font, bool a_UseCache, const uint32_t * a_HashNameP )
{
	if ( a_Font == nullptr )
	{
		ASSERT_WARN( false, "Failed to resolve text texture! No font specified!" );
		return Texture();
	}
	...
	identifier.Value0 = CRC32( a_Text.Get() );
	...
	if ( !cttdPtrQuery )
	{
		...
		Texture texture = LoadTexture( a_HashNameP 
											? ( *a_HashNameP )
											: CRC32( CString::Format( "TextTexture_%llu%u", identifier.Value0, identifier.Value1 ).Get() ), 
...
void GraphicsManager::SetState( RenderState a_RenderState, bool a_Value )
{
	...
	case RS_STENCILING:
		if ( a_Value )
		{
			m_StateInfo.Stencil = true;
			glEnable( GL_STENCIL_TEST );
		}
		else
		{
			m_StateInfo.Stencil = false;
	...
	case RS_STENCILING_WRITE:
		if ( a_Value )
		{
			ASSERT_WARN( m_StateInfo.Stencil, "Setting 'RS_STENCILING_WRITE' to true. 'RS_STENCILING' is false; set to true first!" );

			glStencilFunc( GL_ALWAYS, m_StateInfo.StencilReadWriteValues[ 1 ], 0x00 );
		...
		{
			glStencilFunc( GL_EQUAL, m_StateInfo.StencilReadWriteValues[ 0 ], 0xFF );
...
void GraphicsManager::SetState( RenderState a_RenderState, int32_t a_Value )
{
	switch ( a_RenderState )
	{
	case RS_STENCILING_READ:
		m_StateInfo.StencilReadWriteValues[ 0 ] = a_Value & 0xFF;
		break;
	case RS_STENCILING_WRITE:
		m_StateInfo.StencilReadWriteValues[ 1 ] = a_Value & 0xFF;
		break;
	case RS_TRANSPARENCY:
	case RS_STENCILING:
	case RS_ADDITIVE_BLEND:
...
//====================================================================================
void GraphicsManager::SetState( RenderState a_RenderState, const Tuple2< Vector2, Vector2 > & a_Value )
{
	switch ( a_RenderState )
	{
	case RS_SCISSOR:
	{
		static GLint prev[ 4 ];
		static bool lastSet = false;//was glScissor path called previously vs else path?

		if ( a_Value.Value1.x > 0.0F && a_Value.Value1.y > 0.0F )
		{
			// We only capture current scissorbox on the first call since passing zero-val.
			if ( !lastSet )
			{
			...
			lastSet = true;
			glScissor( static_cast< GLint >( a_Value.Value0.x ),
					   static_cast< GLint >( a_Value.Value0.y ),
					   static_cast< GLint >( a_Value.Value1.x ),
					   static_cast< GLint >( a_Value.Value1.y ) );
		}
		else
		{
			lastSet = false;
