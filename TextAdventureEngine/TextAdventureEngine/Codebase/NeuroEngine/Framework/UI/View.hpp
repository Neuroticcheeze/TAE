...
#ifdef _DEBUG
	//!	\brief Flag for displaying view debug information. 
	//		   Only available in debug compilation mode.
	static bool g_DebugViews;
#endif//_DEBUG
...
public:
	...
	inline const Array< View* > & GetChildren() const
	{
		return m_ChildrenViews;
	}

	View * FindChild( const CString & a_Name ) const;
	View * FindDescendant( const CString & a_Path );
	const View * FindDescendant( const CString & a_Path ) const;
	...
	{
		return CRC32( m_Name.Get() ) == CRC32( a_Other.m_Name.Get() );
	}
