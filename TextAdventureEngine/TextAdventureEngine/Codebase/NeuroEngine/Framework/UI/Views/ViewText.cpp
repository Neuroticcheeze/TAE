...
void ViewText::OnTick( float a_DeltaTime )
{
	...
	if ( m_WordClip )
	{
		GraphicsManager::Instance().SetState( GraphicsManager::RenderState::RS_SCISSOR, Tuple2< Vector2, Vector2 >( GetPosition(), GetSize() ) );
	}
	...
	{
		PROFILE2( "Calculating string height" );
...

	{
		...
		while ( it < m_TextLines.End() )
		{
			...
			PROFILE2( "Drawing the text -> Per-Line" );
	...
	if ( m_WordClip )
	{
		GraphicsManager::Instance().SetState( GraphicsManager::RenderState::RS_SCISSOR, Tuple2< Vector2, Vector2 >() );// Passing in zeroes for size will disable our custom scissorbox and restore the viewport one.
	}
}
