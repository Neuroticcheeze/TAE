...
public:
	...
	public:
		...
		T * operator-> () const
		{
			return static_cast< T* >( m_WeakPtr.Ptr() );
		}

		bool operator==( const WeakPointer< TBase > & a_Other ) const
		{
			return m_WeakPtr == a_Other.m_WeakPtr;
		}
		...
		TWeakPointer( const WeakPointer< TBase > & a_WeakPtr )
			: m_WeakPtr( a_WeakPtr )
		{
		}

		TWeakPointer & operator=( const WeakPointer< TBase > & a_WeakPtr )
		{
			m_WeakPtr = a_WeakPtr;
		}
...
using TWeakPointer = GlobalPtrTable::TWeakPointer< T, P >;
