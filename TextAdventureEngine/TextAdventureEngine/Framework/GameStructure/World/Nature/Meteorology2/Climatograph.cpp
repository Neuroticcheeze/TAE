...
//=====================================================================================
Climatograph::Climatograph( const float * a_YearlyDayTempsCelcius,
							const float * a_YearlyNightTempsCelcius,
							uint32_t a_Entries )
{
	for ( uint32_t k = 0; k < a_Entries; ++k )
	{
		const float t = ( a_Entries == 1 ) ? 0.0F : ( static_cast< float >( k ) / ( a_Entries - 1 ) );
		m_Entries.AddKeyFrame( t, Vector2( a_YearlyDayTempsCelcius[ k ], a_YearlyNightTempsCelcius[ k ] ) );
	}
}

//=====================================================================================
Climatograph::Climatograph( const InitialiserList< float > & a_YearlyDayTempsCelcius,
							const InitialiserList< float > & a_YearlyNightTempsCelcius )
	: Climatograph( Begin( a_YearlyDayTempsCelcius ), 
					Begin( a_YearlyNightTempsCelcius ),
					Min( Size( a_YearlyDayTempsCelcius ), Size( a_YearlyNightTempsCelcius ) ) )
{
}
