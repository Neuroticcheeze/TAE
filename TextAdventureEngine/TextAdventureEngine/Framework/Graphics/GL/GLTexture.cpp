	...
	, m_TextureType( a_TextureType )
{
	if ( ASSERT( a_FromSDLSurface, "Texture Error [%s] (Supplied with a NULL pointer to SDL2 Surface)", Name() ) )
	{
		...
		if ( a_TextureType == TextureType::TT_2D )
		{
			...
			uint32_t levels = Ceil( Log2( Max( w, h ) ) );
		...
		else
		{
			...
			glTexParameteri( GL_TEXTURE_1D, GL_TEXTURE_WRAP_S, ( uint32_t )a_Wrap );
			glTexParameteri( GL_TEXTURE_1D, GL_TEXTURE_WRAP_T, ( uint32_t )a_Wrap );
